...
class BatchRNN(nn.Module):
    ...
    def forward(self, x, output_lengths):
        if self.batch_norm is not None:
            ...
            # X is of sixe (T x B X *) ( T is the size of longest seq and B is batch size, * is num of features)
            x = self.batch_norm(x)
...
class DeepSpeech(nn.Module):
    def __init__(self, rnn_type=nn.LSTM, labels="abc", rnn_hidden_size=768, nb_layers=5, audio_conf=None,
        ...
        # rnn input size = amplitudes 161
        rnn_input_size = int(math.floor((sample_rate * window_size) / 2) + 1)

        # input size +2*padding - kernel / stride +1 First conv layer
        rnn_input_size = int(math.floor(rnn_input_size + 2 * 20 - 41) / 2 + 1)

        # input size +2*padding - kernel / stride +1 second conv layer
        rnn_input_size = int(math.floor(rnn_input_size + 2 * 10 - 21) / 2 + 1)

        # multiplied by num of channels
        rnn_input_size *= 32
    ...
    def forward(self, x, lengths):
        ...
        # x is of size (20,32,41, 338)
        sizes = x.size()
        x = x.view(sizes[0], sizes[1] * sizes[2], sizes[3])  # Collapse feature dimension

        # x becomes 338,20,1024
